...
API_="sk-xxxxxxxxxxxxxxxx"
API_URL = "https://api.openai.com/v1/chat/completions"

df = pd.read_csv('IEMOCAP/DATA/Context.csv', encoding='latin-1', quotechar='|')
...
question = "According to the context of the conversation, choose between Greeting, Question, Answer, Statement Opinion, Statement Non Opinion, Apology, Command, Agreement, Disagreement, Acknowledge, Backchannel, and Others. Answer each sentence in one word with a list and corresponding number. Not a summary. Do not give me the reasoning. What dialogical act is shown in each turn of the next conversation?: "

with open("IEMOCAP/RES_IEMO/Context_gets_DAs.csv", "w", newline='') as f:
    my_csvwriter = csv.writer(f)
    my_csvwriter.writerow("Conversation, ChatGPT".split(','))
    conversation = 0

    with open('IEMOCAP/RES_IEMO/Logs/Context_gets_DAs.txt', 'w') as file:
