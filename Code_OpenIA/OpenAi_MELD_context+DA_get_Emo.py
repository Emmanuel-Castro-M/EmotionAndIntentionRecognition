...
API_="sk-xxxxxxxxxxxxxx"
API_URL = "https://api.openai.com/v1/chat/completions"

df = pd.read_csv('MELD/DATA/context+DA.csv', encoding='latin-1', quotechar='|')
pattern = re.escape('**') + "(.*?)" + re.escape('**')

question = "According to the context of the conversation and its dialogical act classification, choose between anger, disgust, fear, joy, neutral, sadness, or surprised. Each sentence has been labeled according to the dialogical act shown in parentheses. Answer each sentence in one word with a list and corresponding number. Not a summary. What emotion is shown in each turn of the conversation?: "

with open("MELD/RES_MELD+DA_emo/Context+DA_gets_Emo.csv", "w", newline='') as f:
    my_csvwriter = csv.writer(f)
    my_csvwriter.writerow("Conversation, ChatGPT".split(','))
    conversation = 0

    with open('MELD/RES_MELD+DA_emo/Logs/Context+DA_gets_Emo.txt', 'w') as file:
